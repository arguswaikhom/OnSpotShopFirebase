export class DistanceUtil {
  ...
  static distance(location1, location2) {
    ...
    const latDelta = DistanceUtil.degreesToRadians(
      location2.latitude - location1.latitude
    );
    const lonDelta = DistanceUtil.degreesToRadians(
      location2.longitude - location1.longitude
    );

    const a =
      Math.sin(latDelta / 2) * Math.sin(latDelta / 2) +
      Math.cos(DistanceUtil.degreesToRadians(location1.latitude)) *
        Math.cos(DistanceUtil.degreesToRadians(location2.latitude)) *
        Math.sin(lonDelta / 2) *
        Math.sin(lonDelta / 2);
  ...
  static boundingBoxCoordinates(center, radius) {
    ...
    const longDegsNorth = DistanceUtil.metersToLongitudeDegrees(
      radius,
      latitudeNorth
    );
    const longDegsSouth = DistanceUtil.metersToLongitudeDegrees(
      radius,
      latitudeSouth
    );
